...
################################### MODEL ######################################################
if model == "EFF":
	from ModelsCtr.EFF import Module,Number,Density,cdf
	#--------- Initial parameters --------------
...
if model == "King":
	from ModelsCtr.King import Module,Number,Density,cdf
	#--------- Initial parameters --------------
...
if model == "RGDP1":
	from ModelsCtr.RGDP1 import Module,Number,Density,cdf
	#--------- Initial parameters --------------
	namepar = ["$\\alpha$","$\\beta$","$\gamma$"]
	params  = [0.5,2.0,0.0]
	hyp     = np.array([2,4])
	rng     = None#[[0,2],[3,40],[0,3]]
	
if model == "GDP":
	from ModelsCtr.GDP import Module,Number,Density,cdf
	#--------- Initial parameters --------------
...
if model == "RGDP2":
	from ModelsCtr.RGDP2 import Module,Number,Density,cdf
	#--------- Initial parameters --------------
	namepar = ["$r_c\ \ [pc]$","$\\alpha$","$\gamma$"]
	params  = [4.0,0.5,0.1]
	hyp     = np.array([10,2])
	rng     = None#[[0,hyp[0]],[0,hyp[1]],[2,hyp[2]]]

if model == "GKing":
	from ModelsCtr.GKing import Module,Number,Density,cdf
	#--------- Initial parameters --------------
...
if model == "OGKing":
	from ModelsCtr.OGKing import Module,Number,Density,cdf
	#--------- Initial parameters --------------
	namepar = ["$r_c\ \ [pc]$","$r_t\ \ [pc]$"]
	params  = [2.0,20.0]
	hyp     = np.array([10,10])
...
nameparCtr = ["$\phi$","$\\alpha_c$","$\\delta_c$"]
hypCtr    = np.array([1.0,56.5,24.0])

namepar = nameparCtr + namepar
hyp     = hypCtr + hyp

if real :
	dir_out  = dir_+'MultiNest/Samples/'+model+'_Ctr_'+str(int(Rmax))
else:
	dir_out  = dir_+'MultiNest/Samples/Synthetic/'+model+'_Ctr_'+str(int(Rmax))+'_'+str(Ntot)
